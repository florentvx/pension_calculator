...
class model_statics:
    def __init__(
        ...
        contribution_increase_month_list: list[int]|list[str],
        ...
        self.contribution_increase_month_set = set([
            mth if isinstance(mth, int) else (
                MONTH_MAP[mth] if isinstance(mth, str) else None
            )
            for mth in contribution_increase_month_list
        ])
...
def simulate_pension_fund(
    ...
    while res[-1].month_id < statics.working_number_years * 12:
        ...
        new_contrib *= 1 + (
            statics.annual_contribution_increase_rate / len(statics.contribution_increase_month_set)
            if add_month(res[-1].date).month in \
                statics.contribution_increase_month_set
            else 0
        )
